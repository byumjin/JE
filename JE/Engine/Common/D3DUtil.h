...
public:
	...
	//	DXGI_FORMAT format = DXGI_FORMAT_UNKNOWN,/*DXGI_FORMAT_FROM_FILE,*/
	//	UINT filter = D3D11_FILTER_MIN_MAG_MIP_POINT  /*D3DX11_FILTER_NONE*/,
	//	UINT mipFilter = D3D11_FILTER_MIN_MAG_MIP_LINEAR /* D3DX11_FILTER_LINEAR*/);
	...
	)
	{
		...
		std::wstring source(filename);
		std::wstring wext;

		size_t startposition;
		startposition = source.rfind(L".", source.length());
		
		wext = source.substr(startposition, source.length());

		LPCWSTR ext = wext.c_str();
...
namespace Colors
{
	...
	XMGLOBALCONST XMVECTORF32 Minus = { -1.0f, -1.0f, -1.0f, -1.0f };
