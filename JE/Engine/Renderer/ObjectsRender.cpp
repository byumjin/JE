			...
			for (UINT j = 0; j < (*li)->GetLayerCount(); j++)
			{
				...
				if (Check_IntersectAxisAlignedBoxFrustum((*li)) != 0)
				{
					...
					if (pEffect)
					{
						...
						(XMLoadFloat4x4(&(*li)->GetWorldMatrix()), XMLoadFloat4x4(&mWorldView), XMLoadFloat4x4(&mViewProj), XMLoadFloat4x4(&mWVP), XMLoadFloat4x4(&mInvView), XMLoadFloat4x4(&mInvWorldTranspo), XMLoadFloat4x4(&mInvViewProj), XMLoadFloat4x4(&mInvProj),
