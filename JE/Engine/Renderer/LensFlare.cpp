...
bool LensFlare::Initialize(ID3D11Device* device, int screenWidth, int screenHeight, int resolutionWidth, int resolutionHeight)
{
	bool result;
...
void LensFlare::Shutdown()
{
	...
	ShutdownBuffers();
	delete m_LensFlareTexture;
	delete m_GaussianVTexture;
	delete m_GaussianHTexture;
	return;
}
void LensFlare::OnResize(int ScreenWidth, int ScreenHeight, int MapWidth, int MapHeight)
{
	//DownSample
	...
	m_HighLightTexture->OnResize(m_bitmapWidth, m_bitmapHeight, pdeviceContext, NULL);
	m_LensFlareTexture->OnResize(m_bitmapWidth, m_bitmapHeight, pdeviceContext, NULL);
...
bool LensFlare::Render(ID3D11DeviceContext* deviceContext, int positionX, int positionY, XMFLOAT4X4 InvViewProj, ID3DX11EffectTechnique* pTech, Effects* pEffects)
{	
	...
	SetBackBufferRenderTarget(m_LensFlareTexture->GetRenderTargetView(), NULL, NULL, NULL, FALSE, TRUE, FALSE, FALSE);
	
	
	

	//activeTech = mEffects.LensFlareFX->LensFlareTech;
	pEffects->LensFlareFX->SetAllVariables(XMLoadFloat4x4(&mViewProj), XMLoadFloat4x4(&(pCamera->GetView())), pRenderTextureManger->mRT_List.at(SCENETEXTURE)->GetShaderResourceView(), m_HighLightTexture->GetShaderResourceView(),
	m_DirtMaskTexture->GetShaderResourceView(), m_StarBurstTexture->GetShaderResourceView(), gap);
	RenderBuffer(pdeviceContext, 0, 0, mInvViewProj, pEffects->LensFlareFX->LensFlareTech, pEffects->LensFlareFX);
	//pEffects->LensFlareFX->SetNulltoTextures();
	
	pdeviceContext->GenerateMips(m_LensFlareTexture->GetShaderResourceView());
	


	SetBackBufferRenderTarget(m_GaussianVTexture->GetRenderTargetView(), NULL, NULL, NULL, FALSE, TRUE, FALSE, FALSE);
	//activeTech = mEffects.LensFlareFX->GaussianVerticalTech;
	pEffects->LensFlareFX->SetAllVariables(XMLoadFloat4x4(&mViewProj), XMLoadFloat4x4(&(pCamera->GetView())), m_LensFlareTexture->GetShaderResourceView(), m_HighLightTexture->GetShaderResourceView(), m_DirtMaskTexture->GetShaderResourceView(), m_StarBurstTexture->GetShaderResourceView(), gap);
	RenderBuffer(pdeviceContext, 0, 0, mInvViewProj, pEffects->LensFlareFX->GaussianVerticalTech, pEffects->LensFlareFX);
	//pEffects->LensFlareFX->SetNulltoTextures();
	...
	//activeTech = mEffects.LensFlareFX->GaussianHeightTech;
	pEffects->LensFlareFX->SetAllVariables(XMLoadFloat4x4(&mViewProj), XMLoadFloat4x4(&(pCamera->GetView())), m_GaussianVTexture->GetShaderResourceView(), m_LensFlareTexture->GetShaderResourceView(), m_DirtMaskTexture->GetShaderResourceView(), m_StarBurstTexture->GetShaderResourceView(), gap);
	RenderBuffer(pdeviceContext, 0, 0, mInvViewProj, pEffects->LensFlareFX->GaussianHeightTech, pEffects->LensFlareFX);
	//pEffects->LensFlareFX->SetNulltoTextures();
...
void LensFlare::SetBlendState(bool bAlphaToCoverageEnable, bool bIndependentBlendEnable, bool bBlendEnable,
...
void LensFlare::RenderBuffer(ID3D11DeviceContext* deviceContext, int positionX, int positionY, XMFLOAT4X4 InvViewProj, ID3DX11EffectTechnique* pTech, Effect* pEffect)
{
	bool result;
...
int LensFlare::GetIndexCount()
{
	return m_indexCount;
}


bool LensFlare::InitializeBuffers(ID3D11Device* device)
{
	Vertex* vertices;
...
void LensFlare::ShutdownBuffers()
{
...
XMFLOAT4 LensFlare::CalculateVertex(float X, float Y, XMFLOAT4X4 InvViewProj)
{
...
bool LensFlare::UpdateBuffers(ID3D11DeviceContext* deviceContext, int positionX, int positionY, XMFLOAT4X4 InvViewProj)
{
...
void LensFlare::RenderBuffers(ID3D11DeviceContext* deviceContext, ID3DX11EffectTechnique* pTech, Effect* pEffect)
{
	unsigned int stride;
	unsigned int offset;
...
bool LensFlare::CreateFrameBuffers(ID3D11Device* pd3dDevice)
{
	


	m_LensFlareTexture = new RenderTexture();
	m_LensFlareTexture->Initialize(pd3dDevice, m_bitmapWidth, m_bitmapHeight, 0, 1, DXGI_FORMAT_R8G8B8A8_UNORM,
