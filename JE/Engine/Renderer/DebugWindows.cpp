...
void DebugWindows::PushSwitch(int x)
{
	/*
	for (int i = 0; i < 12; i++)
	{
...
bool DebugWindows::Render(ID3D11DeviceContext* deviceContext, int positionX, int positionY, XMFLOAT4X4 InvViewProj, ID3DX11EffectTechnique* pTech, Effects* pEffects)
{
	...
	//{
		std::list<DirectionalLightActor*>::iterator ppDLi = pDirectionalLightActorManager->mDLA_List.begin();
		DirectionalLightActor* pDL = (*ppDLi);

		pShadowTexture = pDL->m_Shadow_Texture;
	//}
	...
	//{
		...
		pEffects->DebugWindowFx->SetAllVariables(XMLoadFloat4x4(mWorld), XMLoadFloat4x4(&mViewProj), XMLoadFloat4x4(&mWVP), XMLoadFloat4x4(&mInvWorldTranspo),
			...
			pMRTforDeferredR->GetShaderResourceView(2), pMRTforDeferredR->GetShaderResourceView(3), m_HBAOTexture->GetShaderResourceView(),
			pMRTforDeferredR->GetShaderResourceView(4), pRenderTextureManger->mRT_List.at(REFLECTION)->GetShaderResourceView(), m_BloomTexture->GetShaderResourceView(), m_LensFlareTexture->GetShaderResourceView(),
		...
		m_DebugWindow[7].Render(pdeviceContext, m_DebugWindow[7].m_bitmapWidth * 3, m_DebugWindow[7].m_bitmapHeight, mInvViewProj, pEffects->DebugWindowFx->LensFlareTech, NULL);
		...
		m_DebugWindow[11].Render(pdeviceContext, m_DebugWindow[11].m_bitmapWidth * 2, m_DebugWindow[11].m_bitmapHeight * 3, mInvViewProj, pEffects->DebugWindowFx->BloomTech, pEffects);
		

		//m_DebugWindow[9].Render(pdeviceContext, 0, 0, mInvViewProj, pEffects->DebugWindowFx->AOTech, pEffects);
	//}
	///////// ---------> Draw DebugWindows
