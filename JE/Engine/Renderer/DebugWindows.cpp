...
bool DebugWindows::Render(ID3D11DeviceContext* deviceContext, int positionX, int positionY, XMFLOAT4X4 InvViewProj, ID3DX11EffectTechnique* pTech, Effects* pEffects)
{
	...
	//{
		...
		pEffects->DebugWindowFx->SetAllVariables(XMLoadFloat4x4(mWorld), XMLoadFloat4x4(&mViewProj), XMLoadFloat4x4(&mWVP), XMLoadFloat4x4(&mInvWorldTranspo),
			...
			pMRTforDeferredR->GetShaderResourceView(2), pMRTforDeferredR->GetShaderResourceView(3), pMRTforDeferredR->GetShaderResourceView(3),
			pMRTforDeferredR->GetShaderResourceView(4), pRenderTextureManger->mRT_List.at(REFLECTION)->GetShaderResourceView(), pRenderTextureManger->mRT_List.at(SILHOUETTE)->GetShaderResourceView(),
